...
def klines(symbol):
    ...
    else: last_time: now = pd.Timestamp.utcnow().round('H')-pd.Timedelta(hours=2)
    ...
    list_klines = bc.klines(symbol=symbol,interval='1h',startTime=next_time)
    klines.df = klines.binance_to_df(list_klines)
    log.info("{} Ingest kline {}".format(pd.Timestamp.utcnow(),klines.ingest()))
