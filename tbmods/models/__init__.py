...
class Model:
    ...
    def __init__(self,prefix,symbol,period,start,end,features_list):
        ...
        self.name = "{}-{}-{}".format(self.prefix.upper(),self.symbol,datetime.now().strftime("%Y%m%d-%H%M%S"))
        self.init_meta()
    ...
    def update_status(self,update):
        ...
        cache.data["models/{}/status".format(self.prefix)] = self.status
        cache.write()
    ...
    def save_meta(self):
        mongodb = MongoDB()
        mongodb.insert('models',self.prefix,self.meta)
        mongodb.close()
